    ...
    <header className="fixed top-0 left-0 w-full z-50 bg-black/90 backdrop-blur-md border-b border-white/10">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-10">
          {navItems.map((item) => (
            <NavLink key={item.name} to={item.path} className={({ isActive }) => `navLink ${isActive ? "active" : ""}`}>
              {item.name}
            </NavLink>
          ))}
        </nav>
            ...
            <div className="px-6 py-6 flex flex-col gap-6">
              {navItems.map((item) => (
                <NavLink
                  ...
                  className={({ isActive }) => `text-sm uppercase tracking-widest transition ${isActive ? "text-red-600" : "text-white/70 hover:text-white"}`}
                >
                  {item.name}
                </NavLink>
              ))}
